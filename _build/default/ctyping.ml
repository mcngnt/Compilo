...
let rec insert_env env x l = 
	...
	let rec aux li = match li with
		| [] -> [x]
		| VART(s,t,d)::q when s = sx && d = dx -> raise (Error(l, "Variable " ^ sx ^ " is already defined."))
...
let rec current_fun_env_type env l = match env with
	| FUNT(s,vl, t)::q  -> t
	| h::q -> current_fun_env_type q l
	| [] -> raise (Error(l, "Return does not belong to any function."))
...
let rec handle_expr le env = match le with | (l,e) -> begin match e with
	...
	| CST x -> TINT
	| NULLPTR -> TPTR TINT
	| SET_VAR(s,le) -> let t = handle_expr le env in (* if s = "glob" then raise (Error(l, "element of env is:" ^ (print_vdeclt (List.nth env 0)) )); *) begin match get_type_env env s l with
														| tx when tx = t -> t
	...
	| OP1(op, le) -> begin match handle_expr le env with
						| TINT -> begin match op with
										| M_DEREF -> raise (Error(l, "Dereferencing int."))
										| M_ADDR -> TPTR TINT
										| _ -> TINT
								end
						| TPTR t -> begin match op with
										| M_MINUS -> raise (Error(l, "Negative pointer."))
										| M_DEREF -> t
										| M_ADDR -> TPTR (TPTR t)
										| _ -> TPTR t 
								end
	...
	| OP2(op, le1, le2) -> let t1 = handle_expr le1 env in let t2 = handle_expr le2 env in 
							begin match (t1,t2) with
									...
									| (TPTR t1, TPTR t2) when t1 = t2 -> begin match op with
																			| S_SUB -> TPTR t1
																			| _ -> raise (Error(l, "Illegal pointer-pointer operation."))
																		end
									| _ -> raise (Error(l, "Illegal pointer-pointer operation."))
							end
	| CMP(op, le1,le2) -> let t1 = handle_expr le1 env in let t2 = handle_expr le2 env in if t1 != t2 then raise (Error(l, "Can't compare different types.")); TINT
	| EIF(le1,le2,le3) -> let t1 = handle_expr le1 env in let t2 = handle_expr le2 env in let t3 = handle_expr le3 env in if t1 != TINT || t2 != t3 then raise (Error(l, "Uncompatible types involved in the ternary operator.")); t2
...
let rec handle_block vdl lcl env d = match vdl with
	...
	| h::q -> match h with
						...
						| CFUN(l,s,vl,t,lc) -> raise (Error(l, "Function defined in another."))
...
and handle_code lc env d = match lc with | (l,c) -> begin match c with
	...
	| CIF(le, lc1, lc2) -> let t = handle_expr le env in if t != TINT then raise (Error(l, "Non-int type as condition.")) else handle_code lc1 env d; handle_code lc2 env d
	| CWHILE(le, lc) -> let t = handle_expr le env in if t != TINT then raise (Error(l, "Non-int type as condition.")) else handle_code lc env d
	| CRETURN None -> raise (Error(l, "Return needs to return a non-empty expression."))
	| CRETURN (Some le) -> let t = handle_expr le env in if (current_fun_env_type env l) != t then raise (Error(l, "Return type does not correspond to function type."));
end
...
let rec handle_val_dec f env = match f with
	...
	| CFUN(l,s,vl,t,lc)::q -> let newenv = insert_env env (FUNT(s, convert_to_type_dec vl, t)) l in  handle_code lc ((convert_to_vdeclt_dec vl)@newenv) 0 ; handle_val_dec q newenv
